...
EFFECT_HTML_ID_ATTRIBUTE = "doc-effect"
...
class EffectPrefabHtmlGenerator(Exception):
    ...
    def generate_html(self, effect_prefab):
        try:
            ...
            return self.sub_generators[effect_prefab.__class__.__name__].generate_html()
        except Exception as e:
...
class SubAlphaEffectPrefabHtmlGenerator(SubEffectPrefabHtmlGenerator):
    def generate_html(self, effect_prefab):
        try:
            ...
            html.set_attribute("id", EFFECT_HTML_ID_ATTRIBUTE)
            html.add_child(HtmlElement("h6", effect_prefab.__class__.__name__))
            html.add_child(HtmlElement("hr"))
            dl = HtmlElement("dl")
            dt = HtmlElement("dt", "Alpha value")
...
class SubShaderEffectPrefabHtmlGenerator(SubEffectPrefabHtmlGenerator):
    ...
    def generate_html(self, effect_prefab):
        try:
            ...
            html.add_child(HtmlElement("hr"))
            dl = HtmlElement("dl")
            dt = HtmlElement("dt", "Shader program")
            dd = HtmlElement("dd")
