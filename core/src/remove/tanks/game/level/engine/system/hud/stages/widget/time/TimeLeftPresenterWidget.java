...
public final class TimeLeftPresenterWidget implements Widget {
    ...
    public void update(float delta, ResourceRegistry resourceRegistry) {
        Locale locale = resourceRegistry
                .getResource(LevelResource.Locale.toString(), Locale.class);
        Properties properties = resourceRegistry
        ...
        valueLabel.setText(
                timeFormatter.formatTime(
                        properties.getFloat(LevelProperty.LevelTimeLeft.getName())
                ).toUpperCase()
        );
    }
}
